...
train <- function() {
    ...
    if (!is.null(training_params$penalty)) {
        penalty <- as.numeric(training_params$penalty)}
    else {
        penalty <- 2}
    ...
    model <- mars(x=training_X, y=training_data[, target], penalty=penalty, degree=degree)
